...
function isString(value) {
  ...
  if (typeof value === 'object') {
    ...
    if (typeof value[0] === 'string' || typeof value.valueOf() === 'string') {
      return true;
    }
  }
  return false;
...
function formatTime(minutes, seconds) {
  return seconds
    .toString()
    .padStart(5, ':'.padStart(3, minutes.toString().padStart(2, '0')));
}
...
function isPalindrome(str) {
  const STR = str
    ...
    .replace('?', '')
    .replace(/,/g, '');
  return STR.includes(STR.split('').reverse().join(''));
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  let result = '';
  result = arr[0].split('').reverse().join('');
...
function invertCase(str) {
  ...
  const newStr = toggledWords.join(' ');
  return newStr;
}
